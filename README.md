...
npm test         # 37 tests, no dependencies beyond Node's own runner
npm run build
```
...
37 tests, zero dependencies, using Node's built-in runner.
...
- `test/design-system.test.mjs` — the design system as executable tests:
  retired palettes stay retired, the only gradient is a functional mask, the
  JS and CSS palettes cannot drift apart, and every text colour clears WCAG AA.
