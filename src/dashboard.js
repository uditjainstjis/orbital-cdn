...
/**
 * The comparison the whole product rests on, against third-party numbers.
 * Nothing here is generated by this simulator.
 */
function groundingPanel(s) {
  const rows = CITIES.map(c => {
    const m   = measuredFibre(c.city)
    const o   = TERRESTRIAL_ORIGINS.reduce((b, x) => {
      const d = haversine(c.lat, c.lon, x.lat, x.lon)
      return (!b || d < b.d) ? { ...x, d } : b
    }, null)
    const obs = s.byCity.find(r => r.key === c.city)
    return { city: c.city, km: Math.round(o.d), m, model: Math.round(modelledFibreMs(c, 0)), obs }
  }).sort((a, b) => a.km - b.km)

  return `
  <table class="dash-table ground-table">
    <thead><tr>
      <th>Origin</th><th>km to region</th><th>Measured fibre</th><th>Azure P50</th>
      <th>My model</th><th>Orbital p50</th><th>Wins</th>
    </tr></thead>
    <tbody>${rows.map(r => `
      <tr>
        <td>${r.city}</td>
        <td>${num(r.km)}</td>
        <td><b>${r.m ? `${r.m.rtt} ms` : '—'}</b></td>
        <td class="muted-cell">${r.m?.azure != null ? `${r.m.azure} ms` : '—'}</td>
        <td class="muted-cell">${r.model} ms</td>
        <td>${r.obs?.p50 ? `${r.obs.p50} ms` : '—'}</td>
        <td class="${r.obs && r.obs.winRate > 0.5 ? 'good' : 'bad'}">${r.obs ? pct(r.obs.winRate) : '—'}</td>
      </tr>`).join('')}
    </tbody>
  </table>
  <p class="dash-note" style="margin-top:14px">
    <b>Measured fibre</b> is ICMP RTT between WonderNetwork VPS nodes (wondernetwork.com/pings, sampled 2026-08-22),
    datacentre-to-datacentre — it excludes consumer last-mile, so it is a <b>conservative</b> baseline and the orbital
    advantage shown here is understated. <b>Azure P50</b> is Microsoft's published 30-day median over their private
    backbone (July 2026); where it disagrees with the public-Internet figure the two are measuring different networks.
    <b>My model</b> is what this simulator would predict from physics alone — shown so the gap is visible rather than hidden.
  </p>
  ${(() => {
    // Read the crossover off the live data rather than restating a fixed number
    const withObs = rows.filter(r => r.obs && r.obs.n >= 5)
    const losers  = withObs.filter(r => r.obs.winRate < 0.5)
    const winners = withObs.filter(r => r.obs.winRate >= 0.5)
    if (!losers.length || !winners.length) return ''
    const lastLose = losers[losers.length - 1]
    const firstWin = winners[0]
    return `<p class="dash-note" style="margin-top:10px">
      <b>External check.</b> Chaudhry &amp; Yanikomeroglu (arXiv:2203.00154) put the LEO-vs-fibre crossover at
      <b>4,472 km</b> for a 550 km shell against idealised fibre, and note it shortens against realistically routed
      fibre. This simulator, using measured baselines, still loses at <b>${lastLose.city}</b>
      (${num(lastLose.km)} km, ${pct(lastLose.obs.winRate)}) and wins from <b>${firstWin.city}</b>
      (${num(firstWin.km)} km, ${pct(firstWin.obs.winRate)}) upward — placing its crossover in the same region,
      arrived at independently. Handley (HotNets '18) reports the same effect above ~3,000 km for a higher shell.
    </p>`
  })()}`
}
...
export function renderDashboard() {
  ...
  host.innerHTML = `
    ...
    <div class="dash-card">
      <div class="dash-card-head"><h3>Grounding — measured against third-party data</h3>
        <span class="dash-card-sub">nothing in this table is generated by this simulator</span></div>
      ${groundingPanel(s)}
    </div>
